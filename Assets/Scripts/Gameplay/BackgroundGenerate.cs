...
public class BackgroundGenerate : MonoBehaviour
{
    ...
    void loadChildObjects(GameObject obj)
    {
        ...
        for (int i = 0; i <= childsNeeded; i++)
        {
            ...
            float yPos = obj.transform.position.y + (objectHeight * i);
            if (i == childsNeeded)
            {
                yPos -= choke; // Precision adjustment for edge to egede
            }
            c.transform.position = new Vector3(obj.transform.position.x, yPos, obj.transform.position.z);
            c.name = obj.name + i;
        }
        Destroy(clone);
    ...
    void repositionChildObjects(GameObject obj)
    {
        ...
        if (children.Length > 1)
        {
            ...
            if (transform.position.y + screenBounds.y > lastChild.transform.position.y + halfObjectHeight)
            {
                ...
                firstChild.transform.position = new Vector3(firstChild.transform.position.x, lastChild.transform.position.y + halfObjectHeight * 2, lastChild.transform.position.z);
            }
            else if (transform.position.y - screenBounds.y < firstChild.transform.position.y - halfObjectHeight)
            {
                lastChild.transform.SetAsFirstSibling();
                lastChild.transform.position = new Vector3(lastChild.transform.position.x, firstChild.transform.position.y - halfObjectHeight * 2, firstChild.transform.position.z);
            }
        }
    }
